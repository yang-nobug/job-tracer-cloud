...
@media (max-width: 820px) {
  :global(.source-ingest-dialog) { width: 100% !important; height: 100dvh; max-height: 100dvh; margin: 0 !important; border-radius: 0; }
  :global(.source-ingest-dialog .el-dialog__header) { margin-right: 0; padding: 17px 48px 14px 16px; border-bottom: 1px solid var(--jt-line, #e7e9ee); }
  :global(.source-ingest-dialog .el-dialog__body) { max-height: calc(100dvh - 76px); padding: 14px 12px calc(18px + env(safe-area-inset-bottom)); overflow-y: auto; }
  .dialog-heading { padding-right: 0; }.heading-step { display: none; }
  .ingest-progress { grid-template-columns: 1fr 20px 1fr; margin-bottom: 15px; padding: 11px; }
  .progress-step { grid-template-columns: 24px auto; column-gap: 7px; }.progress-step span { width: 23px; height: 23px; }.progress-step b { font-size: 12px; }.progress-step small { font-size: 10px; }
  .step-body { min-height: 0; }.material-grid, .source-meta, .confirm-row { grid-template-columns: 1fr; }
  .source-meta { gap: 11px; }.meta-field :deep(.el-radio-group) { display: flex; width: 100%; }.meta-field :deep(.el-radio-button) { flex: 1; }.meta-field :deep(.el-radio-button__inner) { width: 100%; padding-inline: 7px; }
  .material-panel { padding: 12px; }.text-area :deep(textarea) { min-height: 170px !important; }.drop-zone, .image-panel { min-height: 180px; }
  .step-actions { position: sticky; bottom: -14px; margin: 14px -12px 0; padding: 10px 12px calc(10px + env(safe-area-inset-bottom)); background: rgba(255,255,255,.96); border-top: 1px solid #e7e9ee; }.step-actions :deep(.el-button) { min-height: 40px; }
  .action-hint { display: none; }.step-actions :deep(.el-button:last-child) { flex: 1; }
  .cand-list { max-height: none; overflow: visible; padding-right: 0; }.cand-row { gap: 7px; padding: 10px; flex-wrap: wrap; }.cand-main { order: 1; flex-basis: calc(100% - 31px); }.cand-check { order: 0; padding-top: 22px; }.category-select { order: 2; width: 110px; margin-top: 0; }.remove-row { order: 3; margin: 0 0 0 auto; }
  .entry-intro > span { display: none; }
}
</style>
