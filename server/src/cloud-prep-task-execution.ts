...
async function stage<T>(prompt: string, schemaName: string, schema: Record<string, unknown>, input: unknown, validate: (value: unknown) => T, state: ReturnType<typeof metrics>): Promise<T> {
  const text = JSON.stringify(input); if (text.length > 160000) throw new CloudPrepTaskError('准备任务上下文过长，请精简 JD 或知识资料')
  const result = await completeStructured([{ role: 'system', content: `${loadPrompt(prompt)}\n\nJSON Schema:\n${JSON.stringify(schema)}` }, { role: 'user', content: `<untrusted_task_context_json>\n${text}\n</untrusted_task_context_json>` }], { task: 'interviewPrepAgent', schemaName, schema, validate, maxOutputTokens: 8192, timeoutMs: 180000, skipAudit: true }); measure(state, result.completion); return result.value
}
...
async function moduleStage(taskContext: Awaited<ReturnType<typeof context>>, blueprint: PrepTaskBlueprint, descriptor: PrepTaskBlueprint['modules'][number], valid: Set<string>, state: ReturnType<typeof metrics>, criticFeedback: PrepTaskCriticResult['issues'] = []): Promise<PrepLearningModule> {
  return stage('prep-task-module.system.md', 'prep_task_learning_module_v2', PREP_LEARNING_MODULE_SCHEMA as any, { task_context: taskContext, course_overview: blueprint.overview, course_objectives: blueprint.objectives, module: descriptor, critic_feedback: criticFeedback }, value => moduleEvidence(value, descriptor, valid), state)
}
async function practiceStage(taskContext: Awaited<ReturnType<typeof context>>, blueprint: PrepTaskBlueprint, modules: PrepLearningModule[], state: ReturnType<typeof metrics>, criticFeedback: PrepTaskCriticResult['issues'] = []): Promise<PrepTaskPracticeSet> {
  return stage('prep-task-practice.system.md', 'prep_task_practice_v2', PREP_TASK_PRACTICE_SCHEMA as any, { task: taskContext.task, overview: blueprint.overview, objectives: blueprint.objectives, coverage_map: blueprint.coverage_map, modules, critic_feedback: criticFeedback }, value => validatePrepTaskPracticeSet(value, new Set(modules.map(module => module.id))), state)
}
...
async function generate(taskId: number, workspaceId: string, sessionId: number, force: boolean) {
  ...
  try {
    ...
    const blueprint = await stage('prep-task-blueprint.system.md', 'prep_task_blueprint_v2', PREP_TASK_BLUEPRINT_SCHEMA as any, { task_context: taskContext }, value => { const plan = validatePrepTaskBlueprint(value); plan.modules.forEach(module => module.evidence_refs.forEach(ref => { if (!valid.has(ref)) throw new CloudPrepTaskError(`课程引用无效：${ref}`) })); return plan }, state)
    const modules: PrepLearningModule[] = []; await updateGeneration(sessionId, 'modules', 20, state)
    for (const descriptor of blueprint.modules) { modules.push(await moduleStage(taskContext, blueprint, descriptor, valid, state)); await updateGeneration(sessionId, 'modules', 20 + Math.round(45 * modules.length / blueprint.modules.length), state) }
    await updateGeneration(sessionId, 'practice', 68, state); let practice = await practiceStage(taskContext, blueprint, modules, state)
    ...
    let critic = await stage('prep-task-critic.system.md', 'prep_task_course_critic_v2', PREP_TASK_CRITIC_SCHEMA as any, { guide: draft }, validatePrepTaskCritic, state)
    let repaired = false
    if (critic.verdict === 'revise') {
      ...
      for (const moduleId of moduleIds) { const index = modules.findIndex(module => module.id === moduleId); const descriptor = blueprint.modules.find(module => module.id === moduleId); if (index >= 0 && descriptor) { modules[index] = await moduleStage(taskContext, blueprint, descriptor, valid, state, critic.issues.filter(issue => issue.module_id === moduleId)); repaired = true } }
      const practiceIssues = critic.issues.filter(issue => issue.target === 'practice' || issue.target === 'guide')
      if (practiceIssues.length) { practice = await practiceStage(taskContext, blueprint, modules, state, practiceIssues); repaired = true }
      ...
      critic = await stage('prep-task-critic.system.md', 'prep_task_course_critic_v2', PREP_TASK_CRITIC_SCHEMA as any, { guide: draft }, validatePrepTaskCritic, state)
    }
